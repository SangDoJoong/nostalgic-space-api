...
import api.map.map_crud as map_crud
from config.database_init import get_db

from .map_schema import MapCreate, MapResponse
...
def map_marker_create(map_data: MapCreate, db: Session = Depends(get_db)):
    db_map = map_crud.create_map_marker(db, map_data)
    if not db_map:
...
def map_marker_get(map_id: int, db: Session = Depends(get_db)):
    map_marker = map_crud.get_map_marker(db, map_id)
    if not map_marker:
        raise HTTPException(status_code=404, detail="Map marker not found")
    return map_marker


@router.get("/content/{content_id}", response_model=MapResponse)
def marker_by_content_get(content_id: int, db: Session = Depends(get_db)):
    map_marker = map_crud.get_marker_by_content(db, content_id)
    if not map_marker:
        raise HTTPException(status_code=404, detail="Marker for content not found")
    return map_marker
