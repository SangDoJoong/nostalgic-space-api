...
async def upload_images(_user_id: int = Form(...), db: Session = Depends(get_db),files: List[UploadFile] = File(...)):

    image_ids = []
    for file in files:
        try:
            saved_file_path = await save_file(file)
            _image_create = ImageCreate(user_id = _user_id,image_address=saved_file_path)
            image_id = image_crud.create_user_image(db=db, image_create=_image_create)
